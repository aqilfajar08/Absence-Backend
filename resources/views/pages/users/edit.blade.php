...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-black/5 p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Perbarui informasi data karyawan.</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-black/5 overflow-hidden">
        <form action="{{ route('user.update', $user->id) }}" method="POST" class="p-6 sm:p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <div class="relative">
                        <select name="department" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-maroon/20 focus:border-brand-maroon transition duration-200 appearance-none bg-white">
                            <option value="">-- Pilih Departemen --</option>
                            @foreach (['ACC', 'FINANCE', 'GA', 'HR-GA-HSE', 'Legal', 'Magang', 'OB', 'OPS Support', 'Ops-Acc', 'Ops-Mtc', 'Pro-log', 'Sec'] as $dept)
                ...
                {{-- Gaji Pokok --}}
                <div class="col-span-2 md:col-span-1">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Gaji Pokok (Harian)</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <span class="text-gray-500 sm:text-sm">Rp</span>
                        </div>
                        <input type="number" name="gaji_pokok" value="{{ old('gaji_pokok', $user->gaji_pokok) }}" min="0"
                               class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-maroon/20 focus:border-brand-maroon transition duration-200">
                    </div>
                    @error('gaji_pokok')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                {{-- Tunjangan --}}
                <div class="col-span-2 md:col-span-1">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tunjangan (Harian)</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <span class="text-gray-500 sm:text-sm">Rp</span>
                        </div>
                        <input type="number" name="tunjangan" value="{{ old('tunjangan', $user->tunjangan) }}" min="0"
                               class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-maroon/20 focus:border-brand-maroon transition duration-200">
                    </div>
                    @error('tunjangan')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Password Baru <span class="text-gray-400 font-normal">(Kosongkan jika tidak ingin mengubah)</span></label>
